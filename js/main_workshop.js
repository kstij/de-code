const mworkshopcard = [
  {
    ...
    workshoplink: "workshop/event1.html", //workshop/git and github/git-and-github.html
  },
  {
...
function display_upcoming_events() {
  ...
  const upcomingEvents = mworkshopcard.filter(
...
function display_past_events() {
  ...
  eventsContainer.innerHTML = "";
  const today = new Date();
  const pastEvents = mworkshopcard.filter(
    (event) => new Date(event.date) < today
  );
